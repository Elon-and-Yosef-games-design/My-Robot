    ...
    public class RangeFunction : IFunction
    {
        public object Apply(IContext context, IList<object> arguments, IDictionary<string, object> namedArguments)
        {
            if (arguments.Count == 1)
                return new Language.Range(Numbers.ToInteger(arguments[0]));

            if (arguments.Count == 2)
                return new Language.Range(Numbers.ToInteger(arguments[0]), Numbers.ToInteger(arguments[1]));

            if (arguments.Count == 3)
                return new Language.Range(Numbers.ToInteger(arguments[0]), Numbers.ToInteger(arguments[1]), Numbers.ToInteger(arguments[2]));
